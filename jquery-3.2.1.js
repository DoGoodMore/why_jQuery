 ...
 */
function assert( fn ) {//1226引用 定义assert
	//assert 断言 即调试器的函数
	var el = document.createElement("fieldset");//在内存中创建一个fieldset标签

	try {
		return !!fn( el );//测试函数是否可用 如果可用返回true
	} catch (e) {
		return false;//不可用则返回false
	} finally {
 ...
 */
//Sizzle就是jQuery中的find方法
	//find() 方法获得当前元素集合中每个元素的后代，通过选择器、jQuery 对象或元素来筛选。
	//find用于寻找一个元素下的对应子元素或后代元素
setDocument = Sizzle.setDocument = function( node ) {//602处有引用
	...
	document = doc;//改变document的指向为全局对象document
	...
	if ( preferredDoc !== document &&//preferredDoc = document(当前文档的document对象)
		...
		(subWindow = document.defaultView) && subWindow.top !== subWindow ) {//subWindow指向传入节点的document对象
//document.defaultView在浏览器中，该属性返回当前 document 对象所关联的 window 对象，如果没有，会返回 null。
	...
	*/
	support.attributes = assert(function( el ) {//添加attributes方法
		el.className = "i";
	...
	});//同理检测getElementsByName方法是否可用且返回值是否为一个数组

	// ID filter and find  译 : ID过滤并查找
	if ( support.getById ) {//如果getElementsByName方法可用
		Expr.filter["ID"] = function( id ) {//Sizzle.selector.filter添加ID方法
			///\\\\([\da-f]{1,6}[\x20\t\r\n\f]?|([\x20\t\r\n\f])|.)/gi
		...
		Expr.find["ID"] = function( id, context ) {//Sizzle.selector.find添加ID方法
			//对元素的getElementById方法进行判定
	...
	} else {//表示不支持getElementByName方法的处理方式
		Expr.filter["ID"] =  function( id ) {
			var attrId = id.replace( runescape, funescape );//对传入的id值进行加工
			return function( elem ) {//然后返回一个函数
				//返回的函数用于检测getAttributeNode是否可用 如果可用就为true 如果不可用就返回false
		...
		Expr.find["ID"] = function( id, context ) {//定义find传入ID的函数
			...
			if ( typeof context.getElementById !== "undefined" && documentIsHTML ) {
				...
				var node, i, elems,
					//将elem指向为按照ID找到的DOM元素
					elem = context.getElementById( id );

				if ( elem ) {

					// Verify the id attribute 译 : 验证id属性
					node = elem.getAttributeNode("id");//将node指向其ID属性
					...
					//到这就表示elems已然为空或是undefined 即 : getAttributeNode getElementById不可用
					//那么就将elems指向为按照getElementsByName获取的节点
					//getElementsByName --> 可以用来获取元素name属性为参数 以及id属性为参数的节点集合
...
jQuery.find = Sizzle;//定义在1148行的 Sizzle
...
jQuery.fn.extend( {
	...
	filter: function( selector ) {//1282处有定义
...
// A central reference to the root jQuery(document) 译 : 对根jQuery（文档）的中央引用
var rootjQuery,
	...
	init = jQuery.fn.init = function( selector, context, root ) {//定义jQuery.fn.init函数 该函数需要三个参数 1. 传入的选择器字符串 2.传入的context上下文 3.传入的全局对象
		var match, elem;
		...
		root = root || rootjQuery; //指正root的指向 即root的值为传入的root的值 如果不传就使其指向document
		...
		if ( typeof selector === "string" ) {//对selector进行判定
			if ( selector[ 0 ] === "<" &&//当selector是一个标签体
				...
				// Assume that strings that start and end with <> are HTML and skip the regex check 译 : 假设用<>开始和结束的字符串是HTML，并跳过正则表达式检查
			...
			// Match html or make sure no context is specified for #id 译 : 匹配html或确保没有为#id指定上下文
			if ( match && ( match[ 1 ] || !context ) ) {//对match进行判定 即确保selector是一个标签体或为#id的字符串 context = undefined

				// HANDLE: $(html) -> $(array)
				if ( match[ 1 ] ) {//match[ 1 ]为true时即使selector为标签体字符串的时候 match[ 1 ] = 字符串
					context = context instanceof jQuery ? context[ 0 ] : context;//对context进行取值判定 判断它是不是jQuery的实例对象 如果是的话就转为DOM元素 如果不是的话就直接使用

					// Option to run scripts is true for back-compat 译 : 对于后台兼容，运行脚本的选项是正确的
					// Intentionally let the error be thrown if parseHTML is not present 译 : 如果parseHTML不存在，故意让错误被抛出
					jQuery.merge( this, jQuery.parseHTML(//执行jQuery.merge(合并两个数组)
						match[ 1 ],//选择器字符串
...
//如果传入的选择器是一个对象(DOM元素) 那么就将其添加到nodes中
			...
			// Convert html into DOM nodes 译 : 将html转换为DOM节点
				...
				tag = ( rtagName.exec( elem ) || [ "", "" ] )[ 1 ].toLowerCase();//对传入的选择器字符串尽心解析并抓取其中的标签名
				//如果标签名不能被正常抓取则使用空串
...
// context (optional): If specified, the fragment will be created in this context, 译 : context（可选）：如果指定，将在此上下文中创建片段，
// defaults to document  译 : 默认指向document
// keepScripts (optional): If true, will include scripts passed in the html string 译 : keepScripts（可选）：如果为true，将包含在HTML字符串中传递的脚本
	...
	if ( typeof context === "boolean" ) {//这条判断的意义为如果没传第二个参数即(context)且传入第三个参数即keepScripts,则将context的值指向为false
		keepScripts = context;//
		context = false;
	}
	...
	parsed = rsingleTag.exec( data );//解析传入的选择器字符串是否为一个单标签(无属性无内容)
